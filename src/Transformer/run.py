...
def train(train_data, dev_data, model, model_par, criterion, optimizer):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        logging.info(f"Epoch: {epoch}, train_loss: {train_loss:.3f}, val_loss: {dev_loss:.3f}, Bleu Score: {bleu_score:.2f}, Used_time:{time_used//60:.0f}m{time_used%60:.0f}s")
...
def run():
    ...
    # 开始训练
    train_losses, val_losses,scores = train(train_dataloader, dev_dataloader, model, model_par, criterion, optimizer)
    # test(test_dataloader, model, criterion)

    plt.figure(figsize=(14, 5))
    plt.subplot(1, 2, 1)
    plt.plot(range(1,config.epoch_num+1),train_losses,label='Train Loss',color='red')
    plt.plot(range(1,config.epoch_num+1),val_losses,label='Validation Loss',color='blue')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1,config.epoch_num+1),scores,label='BLEU Score',color='red')
    plt.xlabel('Epochs')
    plt.ylabel('BLEU Score')
    plt.title("BLEU Score curves")
    plt.legend()
    plt.savefig(f'training_validation_plots.png')


# 多GPU下设置随机种子（保证各GPU上的数据和参数初始化一致）
def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(seed)  # 多GPU必加
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=config.epoch_num, help="训练轮次") 
    parser.add_argument("--gpu-id", type=str, default="0", help="GPU设备ID") 
    parser.add_argument("--output-dir", type=str, default="./weights", help="模型保存目录")  # args.output_dir
    args = parser.parse_args()

    set_seed(args.seed)

    # 覆盖 config.py 中的默认值（后续训练用 args 中的参数）
    ...
    log_file = os.path.join(config.output_dir, "train_log.txt")  # 日志文件路径：output_dir下的train_log.txt
    ...
    logging.basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        handlers=[
            logging.FileHandler(log_file, mode="a", encoding="utf-8"),  # 日志写入到output_dir目录
            logging.StreamHandler()  # 同时输出到控制台
        ]
    )


    # 测试：日志会写入到 config.output_dir 下的文件
    logging.info(f"日志文件已保存到：{log_file}")
    logging.info(f"当前output_dir：{config.output_dir}")
    logging.info(f"PyTorch CUDA版本:{torch.version.cuda}")
    logging.info(f"CUDA是否可用:{torch.cuda.is_available()}")
    logging.info(f"可用GPU数量:{torch.cuda.device_count()}")
    logging.info(f"PyTorch CUDA编译版本:{torch.version.cuda}")
    logging.info(f"当前使用的GPU:{torch.cuda.current_device()}")
